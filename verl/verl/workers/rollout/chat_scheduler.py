...
class ChatCompletionScheduler:
    def __init__(
        ...
        self.api_key = self.config.get("api_key", None) or os.environ.get("VLLM_ROLLOUT_API_KEY")
    ...
    def _get_api_key(self) -> str:
        if not self.api_key:
            raise ValueError(
                "VLLM rollout API key is required. Set actor_rollout_ref.rollout.api_key or VLLM_ROLLOUT_API_KEY."
            )
        return self.api_key
