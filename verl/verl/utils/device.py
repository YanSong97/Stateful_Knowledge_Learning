...
def _is_cuda_available() -> bool:
    """Check the availability of CUDA with error handling.
    
    This function wraps torch.cuda.is_available() to handle cases where
    CUDA initialization may fail or produce warnings (e.g., Error 802).
    """
    try:
        return torch.cuda.is_available()
    except Exception as e:
        logger.warning(f"Failed to check CUDA availability: {e}. Assuming CUDA is not available.")
        return False
