...
CONFIG_NAME="value_lpm_chess_loop_trainer.yaml"
CONFIG_PATH=$ROOT_DIR/src/config/chesspuzzle/$CONFIG_NAME
AGENT_TYPE="NLTS_BFS"    # MDP, LPM

MODEL_PATH=${MODEL_PATH:?Set MODEL_PATH to the model directory before running this script.}
...
EACH_SUMMARY_RESPONSE_LENGTH=2048

AGENT_LOOP_CONFIG_PATH=$ROOT_DIR/src/config/chesspuzzle/lpm_loop_config.yaml
...
# ray stop on script exit (normal exit, Ctrl+C, or SIGTERM)
cleanup() {
    echo "Stopping Ray..."
...
EXPERIMENT_NAME="EXP_800_N3H2_RandomOppo_NoIncludeSimulation_LR1e-6_${AGENT_TYPE}_${MODEL_TAG}_turn${MAX_STEPS}_${DATA_TAG}_${TIME_TAG}"
...
python -m src.train_chess \
    ...
    actor_rollout_ref.rollout.multi_turn.envs.env_config.opponent_mode=random \
    ...
    actor_rollout_ref.rollout.multi_turn.random_move_when_invalid_plan_simulation=True \
    ...
    actor_rollout_ref.rollout.multi_turn.final_reward_readjustment=True \
    actor_rollout_ref.rollout.multi_turn.include_simulation_in_summary=True \
    actor_rollout_ref.rollout.multi_turn.include_turn2_simulation_in_response=False \
