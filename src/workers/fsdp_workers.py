...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        # sft optimizer
        if self.config.actor.sft_loss.enable:
            sft_optim_config = self.config.actor.sft_loss.optim_cfg
            sft_optimizer = optim.AdamW(
                actor_module_fsdp.parameters(),
                lr=sft_optim_config.lr,
                betas=sft_optim_config.get("betas", (0.9, 0.999)),
                weight_decay=sft_optim_config.get("weight_decay", 1e-2),
            )
        else:
            sft_optimizer = None

        # sft lr scheduler will be setup when calling sft fit
        sft_lr_scheduler = None

        return actor_module_fsdp, actor_optimizer, actor_lr_scheduler, actor_model_config, sft_optimizer, sft_lr_scheduler
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_name == "hf":
            ...
            if rollout_mode == "async":     # not using searchRollout directly
                rollout_kwargs = {}
            ...
            elif rollout_mode == 'sync':

                if not use_search:
                    # no search tool involved, switch to normal mode
                    ...
                    from src.workers.rollout.vllm_rollout.vllm_search_rollout_spmd import vLLMSearchRollout
                    rollout_kwargs = {}
                    rollout = vLLMSearchRollout(
                                model_path=local_path,
                                config=rollout_config,
                                tokenizer=self.tokenizer,
                                model_hf_config=self.actor_model_config,
                ...
                raise NotImplementedError(f"Rollout mode {rollout_mode} not supported")
    ...
    def init_model(self):
        ...
        if self._is_actor or self._is_rollout:
            ...
            if self._is_offload_optimizer:
                ...
                if self.sft_optimizer is not None:  
                    offload_fsdp_optimizer(optimizer=self.sft_optimizer)
                    log_gpu_memory_usage("After offload sft optimizer during init", logger=logger)
        ...
        if self._is_actor:
            ...
            self.actor = DataParallelPPOActor(
                config=self.config.actor, actor_module=self.actor_module_fsdp, actor_optimizer=self.actor_optimizer, sft_optimizer=self.sft_optimizer
            )
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def _setup_sft_lr_scheduler(self, total_steps):
        from verl.utils.torch_functional import get_wsd_schedule_with_warmup, get_cosine_schedule_with_warmup

        optim_config = self.config.actor.sft_loss.optim_cfg
        num_warmup_steps = int(total_steps * optim_config.warmup_steps_ratio)
        if not hasattr(optim_config, "lr_scheduler") or optim_config.lr_scheduler == "cosine":
            self.sft_lr_scheduler = get_cosine_schedule_with_warmup(
                optimizer=self.sft_optimizer,
                num_warmup_steps=num_warmup_steps,
                num_training_steps=total_steps
            )
        elif optim_config.lr_scheduler == "wsd":
            self.sft_lr_scheduler = get_wsd_schedule_with_warmup(
                optimizer=self.sft_optimizer,
                num_warmup_steps=num_warmup_steps,
                num_training_steps=total_steps
            )
        else:
            raise NotImplementedError(f"Unknown lr scheduler: {optim_config.lr_scheduler}")


    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    @DistProfiler.annotate(color="red", role="actor_sft_update")
    def update_actor_sft(self, data: DataProto):
        """Update the actor model using SFT (Supervised Fine-Tuning) loss.
        
        This function is similar to update_actor but uses SFT loss instead of PPO policy loss.
        
        Args:
            data (DataProto): A DataProto containing SFT training data with keys:
        ...
        Returns:
            DataProto: A DataProto containing training metrics in meta_info["metrics"]
        """
        # Support all hardwares
        data = data.to(get_device_id())
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data=data)
            # perform training
            with Timer(name="update_sft", logger=None) as timer:
                metrics = self.actor.update_sft(data=data)
            delta_time = timer.last
            global_num_tokens = data.meta_info.get("global_token_num", 0)
            if global_num_tokens > 0:
                estimated_flops, promised_flops = self.flops_counter.estimate_flops(global_num_tokens, delta_time)
                # Use sft_epochs from config if available, otherwise default to 1
                sft_epochs = self.config.actor.get("sft_loss", {}).get("sft_epochs", 1)
                metrics["sft_perf/mfu/actor"] = (
                    estimated_flops * sft_epochs / promised_flops / self.world_size
                )
            metrics["sft_perf/max_memory_allocated_gb"] = get_torch_device().max_memory_allocated() / (1024**3)
            metrics["sft_perf/max_memory_reserved_gb"] = get_torch_device().max_memory_reserved() / (1024**3)
            metrics["sft_perf/cpu_memory_used_gb"] = psutil.virtual_memory().used / (1024**3)

            lr = self.sft_lr_scheduler.get_last_lr()[0]
            metrics["sft_actor/lr"] = lr
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during update_actor_sft", logger=logger)
        if self._is_offload_optimizer:
            offload_fsdp_optimizer(optimizer=self.actor_optimizer)
            log_gpu_memory_usage("After offload actor optimizer during update_actor_sft", logger=logger)
