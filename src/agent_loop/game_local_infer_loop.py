...
def get_env(env_config):
    if env_config.env_name=="ChessPuzzles":
        ...
        return ScienceWorldEnv_Wrapper(env_config=env_config)
...
@register("game_local_infer_agent")
class GameLocalInfer_AgentLoop(AgentLoopBase):
    ...
    def init_class(cls, config, tokenizer, **kwargs):
        ...
        print("Performing class-level GameLocalInfer_AgentLoop initialization")
        ...
        cls.validate_action = config.actor_rollout_ref.rollout.multi_turn.validate_action
    ...
    async def run_mdp(self, messages: list[dict[str, Any]], sampling_params: dict[str, Any], seed) -> AgentLoopOutput:
        ...
        while True:
            ...
            step_request_id = f"{conversation_id}_{step}"
            with simple_timer("generate_sequences", metrics):
                response_ids = await self.server_manager.generate(
                    request_id=step_request_id, prompt_ids=input_ids, sampling_params=sampling_params
                )
                # Apply context limiting if max_window_size is set
                # limited_raw_message = self._limit_context_by_turns(raw_message, self.max_window_size)
                # response_lst = await self.model_api_client.request([limited_raw_message], self.reasoning_effort)      # vllm server receive raw message
                # response_dict = response_lst[0]

            prompt_ids += response_ids
            response_mask += [1] * len(response_ids)
            reward_lst += [0.] * len(response_ids)

            assistant_turns += 1
            # Decode response and check for search query
            response_text = await self.loop.run_in_executor(
                None, lambda: self.tokenizer.decode(response_ids, skip_special_tokens=True)
            )
            turn_message[-1].append({"role": "assistant", "content": response_text})
            ...
            # if not action_text:
            #     break  # No action found, end conversation
            # if not action_valid:
                # print(f"!!! Non-validAction not valid")
            ...
            env_feedback_text = self._format_env_feedback(next_state, self.env.legal_moves_string, step, env_info, action_feedback, task_description=self.env.task_goal if hasattr(self.env, 'task_goal') else "")
        ...
        if self.action_valid_records:
            ...
            # print(f"Action Validity Summary for this run:")
            # print(f"  Total Actions: {len(self.action_valid_records)}")
            # print(f"  Mean Valid Rate: {run_valid_rate:.4f}")
        else:
            pass
            # print(f"No actions recorded in this run")
    ...
    def _extract_action_from_assistant(self, response_text: str):
        ...
        if not match:
            ...
            if not self.validate_action:    # no need to check valid
                return action_content, 1.
    ...
    def _format_env_feedback(self, state: Any, truncated_legal_actions: str, step_idx, env_info: dict, action_feedback: str, task_description: str) -> str:
...
async def _async_main():
    ...
    parser.add_argument("--output", type=str, default="logs/eval/local_infer/infer_logs.jsonl")
    ...
    async def run_one_with_semaphore(seed):
        async with semaphore:
            try:
                local_agent = GameLocalInfer_AgentLoop(
