...
def _load_vllm_judge_config_from_env() -> Dict[str, Any]:
    cfg = dict(VLLM_API_CONFIG)
    urls = os.environ.get("VLLM_JUDGE_URLS") or os.environ.get("VLLM_JUDGE_URL")
    if urls:
        cfg["url"] = [url.strip() for url in urls.split(",") if url.strip()]
    cfg["api_key"] = os.environ.get("VLLM_JUDGE_API_KEY")
    cfg["model_name"] = os.environ.get("VLLM_JUDGE_MODEL_NAME", cfg["model_name"])
    return cfg
...
Do not include long thinking in your response, directly output the judgement.
...
def vllm_inference(prompt: str, inference_cfg):
    if not inference_cfg.get("url"):
        raise ValueError("VLLM judge URL is required. Pass inference_cfg['url'] or set VLLM_JUDGE_URLS.")
    if not inference_cfg.get("api_key"):
        raise ValueError("VLLM judge API key is required. Pass inference_cfg['api_key'] or set VLLM_JUDGE_API_KEY.")
...
def vllm_parse_response(response):
    """
    score 2 for correct, 0 for incorrect, None for invalid
    """

    # clean QwQ thinking response
...
def vllm_evaluate(question: str,
                     answer: str,
                     groundtruth: str,
                     inference_cfg: Dict = None):
    if inference_cfg is None:
        inference_cfg = _load_vllm_judge_config_from_env()
    ...
    vllm_input_prompt = evaluation_prompt.format(
        query=question,
        generated_output=extracted_response,
        ground_truth=groundtruth
    )

    vllm_response = vllm_inference(vllm_input_prompt, inference_cfg)
    score, cleaned_response = vllm_parse_response(vllm_response)

    if do_print:
        print(f"Final score: {score}")
        print(f"llm-as-judge response: {cleaned_response}")
        print(f"========================================================")

    return score, cleaned_response, valid_answer_format, extracted_response, vllm_response


def llm_judge_compute_score(data_source,
                          solution_str,
                          ground_truth,
                          question,
                          model_config,
    ...
    if not question:
        raise NotImplementedError(f"no question for llm-as-judge evaluation")
    else:
        score, evaluation_response, valid_answer_format, extracted_answer, vllm_response = vllm_evaluate(
    ...
    if return_dict:
        return {
            ...
            "do_search": do_search,
            "score_is_none": int(score is None)
        }, evaluation_response, extracted_answer, vllm_response

    return return_score, evaluation_response, extracted_answer, vllm_response
