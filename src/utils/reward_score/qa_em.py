...
def extract_solution(solution_str):
    ...
    if boxed_matches:
        ...
        # 如果内容本身还包含boxed，递归提取
        inner_boxed, valid_answer_format = extract_solution(final_answer)
        if inner_boxed is not None:
            return inner_boxed, valid_answer_format
        else:
            return final_answer.strip(), valid_answer_format
    
    # 如果没有找到标签，尝试其他启发式方法
    lines = solution_str.strip().split('\n')
    if lines:
        return lines[-1].strip(), valid_answer_format
    
    return None, valid_answer_format
...
def compute_score_em(solution_str, ground_truth, method='strict', format_score=0., score=1., return_dict=False, reward_type='em'):
    ...
    # 原来的EM逻辑
    answer, answer_is_valid = extract_solution(solution_str=solution_str)
    ...
    if return_dict:
        return {
            ...
            "valid_answer_format": answer_is_valid,
            "do_search": do_search,
            "correct_without_search": ((not do_search) and (result))
        }
