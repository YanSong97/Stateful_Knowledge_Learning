...
class FrozenLakeEnv(Env):
    ...
    * `is_slippery=True`: If true the player will move in intended direction with probability specified by the
        `success_rate` else will move in either perpendicular direction based on `slip_prob_ratio`.
   ...
   * `slip_prob_ratio=(1.0, 1.0)`: Used to split the failure probability between
        left-slip (`(a - 1) % 4`) and right-slip (`(a + 1) % 4`) when `is_slippery=True`.
        For example, `slip_prob_ratio=(1, 3)` means right-slip is 3x as likely as left-slip.
    ...
    * `reward_schedule=(1, 0, 0)`: Used to specify reward amounts for reaching certain tiles.
    ...
    def __init__(
        ...
        slip_prob_ratio: tuple[float, float] = (1.0, 1.0),
        ...
        left_ratio, right_ratio = slip_prob_ratio
        if left_ratio < 0.0 or right_ratio < 0.0:
            raise ValueError(
                "slip_prob_ratio values must be non-negative, "
                f"got {slip_prob_ratio}"
            )

        fail_total = 1.0 - success_rate
        ratio_sum = left_ratio + right_ratio
        if fail_total > 0.0 and ratio_sum == 0.0:
            raise ValueError(
                "slip_prob_ratio cannot be (0, 0) when success_rate < 1.0"
            )

        left_fail_rate = 0.0 if fail_total == 0.0 else fail_total * (left_ratio / ratio_sum)
        right_fail_rate = (
            0.0 if fail_total == 0.0 else fail_total * (right_ratio / ratio_sum)
        )
            ...
            for col in range(ncol):
                ...
                for a in range(4):
                    ...
                    if letter in b"GH":
                        ...
                        if is_slippery:
                            for b, p in [
                                ((a - 1) % 4, left_fail_rate),
                                (a, success_rate),
                                ((a + 1) % 4, right_fail_rate),
                            ]:
                                li.append(
                                    (
                                        p,
